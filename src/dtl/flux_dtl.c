        ...
        const flux_msg_t* msg, char** upath)
{
    ...
    if (errcode < 0) {
        FLUX_LOG_ERR (
            self->private.flux_dtl_handle->h,
...
dyad_rc_t dyad_dtl_flux_rpc_respond (dyad_dtl_t* self,
...
dyad_rc_t dyad_dtl_flux_send (dyad_dtl_t* self, void* buf, size_t buflen)
{
    ...
    if (errcode < 0) {
        FLUX_LOG_ERR (
            self->private.flux_dtl_handle->h,
            "Could not send Flux RPC response containing file contents\n"
        );

...
dyad_fetch_request_cb (flux_t *h, flux_msg_handler_t *w, const flux_msg_t *msg, void *arg)
{
    ...
    DYAD_LOG_INFO(mod_ctx->ctx, "DYAD_MOD: Launched callback for %s", DYAD_DTL_RPC_NAME);
    ssize_t inlen = 0l;
    char *inbuf = NULL;
    int fd = -1;
    uint32_t userid = 0u;
    ...
    if (file_size > 0l) {
        ...
        DYAD_LOG_DEBUG (mod_ctx->ctx, "DYAD_MOD: Closing file pointer");
        dyad_release_flock (mod_ctx->ctx, fd, &shared_lock);
        close (fd);
        DYAD_LOG_DEBUG (mod_ctx->ctx, "DYAD_MOD: Is inbuf NULL? -> %i", (int)(inbuf == NULL));
    ...
    DYAD_LOG_INFO(mod_ctx->ctx, "DYAD_MOD: Finished %s module invocation\n", DYAD_DTL_RPC_NAME);
    goto end_fetch_cb;

...
#endif
static int
produce_stream (std::ostream& os, const size_t sz)
{
    size_t n = 0ul;
...
#endif
static int
check_read (const char* buf, size_t sz, const std::string& line)
{
    std::string buffer;
    const size_t fill_sz = std::min (static_cast<size_t> (PATH_MAX), sz);
...
#endif
static int
consume_stream (std::istream& ifp, const size_t sz, const bool verify)
{

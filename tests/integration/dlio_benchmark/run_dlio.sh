...
for (( c=0; c<NUM_NODES*BROKERS_PER_NODE; c++ )); do     
  ...
  flux exec -r $c flux module load ${SPACK_ENV}/.spack-env/view/lib/dyad.so --debug --info_log=${DYAD_LOG_DIR}/dyad-broker --error_log=${DYAD_LOG_DIR}/dyad-broker --mode="${DYAD_DTL_MODE}" $DYAD_PATH
done
echo Loaded Modules
flux module list

...
dyad_stream_core::dyad_stream_core()
    : m_ctx(NULL), m_ctx_mutable(NULL), m_initialized(false), m_is_prod(false),
      m_is_cons(false) {
  DYAD_CPP_FUNCTION();
}

dyad_stream_core::~dyad_stream_core() { finalize(); }
...
void dyad_stream_core::init(const bool reinit) {
  ...
  if (reinit || reinit_env || !(m_ctx = m_ctx_mutable = dyad_ctx_get())) {
    ...
    log_info(
        "Stream core skips initialization as it has already been initialized.");
  }
...
void dyad_stream_core::init(const dyad_params &p) {
  ...
  dyad_rc_t rc = dyad_init(
      p.m_debug, false, p.m_shared_storage, p.m_reinit, p.m_async_publish,
      p.m_fsync_write, p.m_key_depth, p.m_key_bins, p.m_service_mux,
      p.m_kvs_namespace.c_str(), p.m_prod_managed_path.c_str(),
      p.m_cons_managed_path.c_str(), p.m_relative_to_managed_path,
...
void dyad_stream_core::log_info(const std::string &msg_head) const {
  ...
  DYAD_LOG_INFO(m_ctx, "%s=%s", DYAD_PATH_CONSUMER_ENV,
                m_ctx->cons_managed_path);
  DYAD_LOG_INFO(m_ctx, "%s=%s", DYAD_PATH_PRODUCER_ENV,
...
bool dyad_stream_core::is_dyad_producer() const { return m_is_prod; }

bool dyad_stream_core::is_dyad_consumer() const { return m_is_cons; }
...
void dyad_stream_core::set_initialized() { m_initialized = true; }

bool dyad_stream_core::chk_initialized() const { return m_initialized; }

bool dyad_stream_core::chk_fsync_write() const { return m_ctx->fsync_write; }
...
std::string dyad_stream_core::get_upath() const { return std::string{upath}; }

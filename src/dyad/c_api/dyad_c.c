...
int dyad_c_init (dyad_ctx_t** ctx)
{
    ...
    rc = dyad_ctx_init (DYAD_COMM_RECV, NULL);
    *ctx = dyad_ctx_get ();
    DYAD_C_FUNCTION_END ();
    return (int)rc;
}
...
real_call:;
    ...
    if ((ret > 0) && (oflag & O_WRONLY || oflag & O_APPEND) && !is_path_dir (path)) {
...
int dyad_c_finalize (dyad_ctx_t** ctx)
{
    ...
    if (ctx == NULL || *ctx == NULL) {
        goto dyad_c_finalize_cleanup;
    }
    DYAD_LOG_INFO (*ctx, "DYAD finalized");
    dyad_ctx_fini ();
    *ctx = NULL;

dyad_c_finalize_cleanup:
    DYAD_C_FUNCTION_END ();
    return (int)rc;
}
...
Options:
  ...
  ${C}--workspace${N} <val>       Pass Merlin workspace directory such that logs files
                          can be dumped into it.
  ${C}--specroot${N} <val>        Pass Merlin specroot directory to help locating other
                          files if neededa.
...
WORKSPACE='.' # Merlin WORKSPACE directory
SPECROOT='.'  # Merlin SPECROOT directory
...
while :; do
    case ${1} in
        -h|--help)
            ...
            ;;
        --workspace)
            # Merlin WORKSPACE directory. Default is the current working dir
            if [ -n "${2}" ]; then
                WORKSPACE=${2}
                shift
            else
            ...
            ;;
        --specroot)
            # Merlin SPECROOT directory. Default is the current working dir
            if [ -n "${2}" ]; then
                SPECROOT=${2}
                shift
            else
...
# Check Merlin workspace directory
if [ -z ${WORKSPACE+x} ] || [ ! -d "${WORKSPACE}" ] ; then
...
# Check Merlin specroot directory
if [ -z ${SPECROOT+x}  ] || [ ! -d "${SPECROOT}" ] ; then
    echo "SPECROOT '${SPECROOT}' does not exist" >&2
    exit 1
elif [ "${SPECROOT}" == "" ] ; then
    SPECROOT='.'
fi
...
if [ "${DYAD_MODULE}" == "" ] ; then
    DYAD_MODULE=${SPECROOT}/dyad/dyad.so
fi
...
print_all_vars
if [ "${DYAD_CONTEXT}" != "" ] ; then
    print_all_vars >  ${WORKSPACE}/vars.${DYAD_CONTEXT}.txt
else
    print_all_vars >  ${WORKSPACE}/vars.txt
fi

...
#endif // URPC_PERFFLOW
static int urpc_rpc_pack (flux_future_t **ft, uint32_t rank, const char *cmd)
{
    // Send the request to execute a command
    if (!(*ft = flux_rpc_pack (ctx->h, "urpc.exec", rank, 0,
                              "{s:s}", "cmd", cmd))) {
...
__attribute__((annotate("@critical_path()")))
#endif // URPC_PERFFLOW
static int urpc_rpc_get_raw (flux_future_t *ft, const char **result,
                      int *result_size)
{

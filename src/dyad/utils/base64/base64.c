 ...
 */
static int8_t sixbit_from_b64 (const base64_maps_t *maps, const unsigned char b64letter)
{
    int8_t ret;
                                  ...
                                  const size_t srclen)
{
    ...
    if (src_offset < srclen) {
        base64_encode_tail_using_maps (maps,
                                       &dest[dest_offset],
                                       &src[src_offset],
                                       srclen - src_offset);
        dest_offset += 4;
                                       ...
                                       const size_t srclen)
{
    ...
    while (insize != 0 && src[insize - 1] == '=') { /* throw away padding symbols */
        insize--;
    }
    if (insize == 0) {
        return 0;

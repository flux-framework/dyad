...
#endif
static void
dyad_fetch_request_cb(flux_t *h, flux_msg_handler_t *w, const flux_msg_t *msg,
                      void *arg) {
  ...
  DYAD_LOG_DEBUG(mod_ctx->ctx,
                 "DYAD_MOD: sending initial response to consumer");

  rc = mod_ctx->ctx->dtl_handle->rpc_respond(mod_ctx->ctx, msg);
  if (DYAD_IS_ERROR(rc)) {
    DYAD_LOG_ERROR(mod_ctx->ctx,
                   "Could not send primary RPC response to client");
    goto fetch_error_wo_flock;
  }
  ...
  if (!get_stat(fullpath, 1000U, 1000L)) {
    DYAD_LOG_ERR(mod_ctx->ctx, "DYAD_MOD: Failed to access info on \"%s\".",
                 fullpath);
    // goto error;
  }
#endif // DYAD_SPIN_WAIT

  DYAD_LOG_INFO(mod_ctx->ctx, "DYAD_MOD: Reading file %s for transfer",
                fullpath);
  fd = open(fullpath, O_RDONLY);

  if (fd < 0) {
    DYAD_LOG_ERROR(mod_ctx->ctx, "DYAD_MOD: Failed to open file \"%s\".",
                   fullpath);
    goto fetch_error_wo_flock;
  }
  ...
  DYAD_LOG_DEBUG(mod_ctx->ctx, "DYAD_MOD: file %s has size %zd", fullpath,
                 file_size);
  rc = mod_ctx->ctx->dtl_handle->get_buffer(mod_ctx->ctx, file_size,
                                            (void **)&inbuf);
  ...
  if (file_size > 0l) {
    ...
    if (DYAD_IS_ERROR(rc)) {
      DYAD_LOG_ERROR(mod_ctx->ctx,
                     "Could not establish DTL connection with client");
      errno = ECONNREFUSED;
      goto fetch_error_wo_flock;
    }
  ...
  DYAD_LOG_DEBUG(mod_ctx->ctx,
                 "Close RPC message stream with an ENODATA (%d) message",
                 ENODATA);
...
fetch_error_wo_flock:;
  DYAD_LOG_ERROR(mod_ctx->ctx,
                 "Close RPC message stream with an error (errno = %d)\n",
                 errno);
...
static int opt_parse(opt_parse_out_t *restrict opt, const unsigned broker_rank,
                     dyad_dtl_mode_t *restrict dtl_mode, int _argc,
                     char **restrict _argv) {
...
dyad_rc_t dyad_module_ctx_init(const opt_parse_out_t *opt,
                               flux_t *h) { // Initialize DYAD context
  dyad_mod_ctx_t *mod_ctx = get_mod_ctx(h);
  ...
  if (opt->dtl_mode) {
    ...
    DYAD_LOG_STDERR("DYAD_MOD: DTL 'mode' option set. "
                    "Setting env %s=%s",
...
DYAD_DLL_EXPORTED int mod_main(flux_t *h, int argc, char **argv) {
  ...
  if (flux_msg_handler_addvec(mod_ctx->ctx->h, htab, (void *)h,
                              &mod_ctx->handlers) < 0) {

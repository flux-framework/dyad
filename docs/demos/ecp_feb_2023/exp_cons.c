 ...
 */
int consume_file (dyad_ctx_t* ctx, char* full_path, int32_t seed, int32_t* val_buf)
{
    // Open the file or abort if not possible
    int fd = dyad_c_open (ctx, full_path, O_RDONLY);
    ...
    ssize_t items_read = read (fd, val_buf, sizeof (int32_t) * NUM_VALS);
    dyad_c_close (ctx, fd);
...
int main (int argc, char** argv)
{
    ...
    for (int32_t seed = 0; seed < num_transfers; seed++) {
         ...
         ********************************************/
        rc = consume_file (ctx, full_path, seed, val_buf);
        if (rc != 0) {
            continue;
        }
